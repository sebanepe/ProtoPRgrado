"""Routes for applying trained supervised models to new datasets (Fase D3)."""
from __future__ import annotations

import os
import shutil
import tempfile
from pathlib import Path
from typing import Optional

from fastapi import APIRouter, BackgroundTasks, Depends, File, Form, HTTPException, Query, UploadFile
from fastapi.responses import JSONResponse
from sqlalchemy.orm import Session

from backend.app.database import get_db
from backend.app.services import supervised_inference_service as svc

router = APIRouter(prefix="/api/supervised-inference", tags=["supervised-inference"])


def _http_error(exc: Exception, status: int = 400) -> HTTPException:
    return HTTPException(status_code=status, detail=str(exc))


# ── Discovery ─────────────────────────────────────────────────────────────────

@router.get("/trained-models")
def get_trained_models(db: Session = Depends(get_db)):
    """List SUPERVISED_HUMAN models with status AVAILABLE."""
    try:
        return svc.list_trained_models(db)
    except Exception as exc:
        raise _http_error(exc, 500)


@router.get("/preprocessed-runs")
def get_preprocessed_runs(db: Session = Depends(get_db)):
    """List completed preprocessing runs (kept for compatibility)."""
    try:
        return svc.list_preprocessed_runs(db)
    except Exception as exc:
        raise _http_error(exc, 500)


@router.get("/rule-summaries")
def get_rule_summaries(db: Session = Depends(get_db)):
    """List available RULE_SUMMARY_CSV artifacts usable as supervised inference input."""
    try:
        return svc.list_rule_summaries(db)
    except Exception as exc:
        raise _http_error(exc, 500)


# ── Apply model ───────────────────────────────────────────────────────────────

@router.post("/apply-trained-model")
def apply_trained_model(
    background_tasks: BackgroundTasks,
    model_registry_id: int = Form(...),
    input_type: str = Form(...),
    preprocessed_run_id: Optional[int] = Form(None),
    file: Optional[UploadFile] = File(None),
    db: Session = Depends(get_db),
):
    """Start a supervised model inference run asynchronously.

    The model is applied over alert summaries generated by the rule engine for the
    selected preprocessed_run. Returns 202 immediately with run_id.
    Poll GET /inference-status/{run_id} until COMPLETED or FAILED.
    Does NOT retrain the model.
    Does NOT generate is_fraud or confirmed_fraud.
    """
    from backend.app.services import artifact_registry_service as art_svc
    from backend.app.models.models import PreprocessingRun

    if input_type not in ("csv_upload", "preprocessed_run"):
        raise HTTPException(
            status_code=422,
            detail="input_type must be 'csv_upload' or 'preprocessed_run'",
        )

    input_file_path: str
    input_source: str

    if input_type == "csv_upload":
        if file is None:
            raise HTTPException(status_code=422, detail="Se requiere un archivo CSV cuando input_type=csv_upload")
        if not file.filename.lower().endswith(".csv"):
            raise HTTPException(status_code=422, detail="El archivo debe ser un CSV (.csv)")

        upload_dir = Path(os.environ.get("PROJECT_PROCESSED_DIR", "")) / "uploads"
        if not upload_dir.parent.exists():
            upload_dir = Path(tempfile.gettempdir()) / "supervised_uploads"
        upload_dir.mkdir(parents=True, exist_ok=True)

        dest = upload_dir / file.filename
        try:
            with open(dest, "wb") as fh:
                shutil.copyfileobj(file.file, fh)
        except Exception as exc:
            raise HTTPException(status_code=500, detail=f"Error al guardar el archivo: {exc}")

        input_file_path = str(dest)
        input_source = file.filename

        try:
            svc.validate_alert_schema_csv(input_file_path)
        except ValueError as exc:
            raise HTTPException(status_code=422, detail=str(exc))

    else:  # preprocessed_run
        if preprocessed_run_id is None:
            raise HTTPException(
                status_code=422,
                detail="Se requiere preprocessed_run_id cuando input_type=preprocessed_run",
            )

        prep_run = db.query(PreprocessingRun).filter(PreprocessingRun.id == preprocessed_run_id).first()
        if prep_run is None or prep_run.status != "COMPLETED":
            raise HTTPException(
                status_code=404,
                detail=f"El preprocessing run {preprocessed_run_id} no existe o no está COMPLETED.",
            )

        source_run = f"preprocessed_run_{prep_run.id}"

        artifact = art_svc.get_artifact_by_type(db, source_run, art_svc.ARTIFACT_RULE_SUMMARY_CSV)
        if artifact is None or not artifact.file_path or not Path(artifact.file_path).exists():
            # Fallback: check disk at the canonical path (artifact may exist but not be registered)
            from backend.app.services.rule_engine_service import PROJECT_PROCESSED_DIR
            run_token_str = str(prep_run.id)
            fallback = Path(PROJECT_PROCESSED_DIR) / f"alerts_summary_run_{run_token_str}.csv"
            if fallback.exists():
                artifact = art_svc.register_or_update_artifact(
                    db,
                    artifact_type=art_svc.ARTIFACT_RULE_SUMMARY_CSV,
                    phase=art_svc.PHASE_B,
                    source_run=source_run,
                    run_token=run_token_str,
                    file_path=str(fallback),
                )
            else:
                raise HTTPException(
                    status_code=404,
                    detail=(
                        "No existen alertas generadas para este run. "
                        "Ejecute primero el motor de reglas para el run seleccionado."
                    ),
                )

        input_file_path = artifact.file_path
        input_source = source_run

    # Validate model exists and is AVAILABLE (after input checks to preserve 422 priority)
    trained = svc.list_trained_models(db)
    if not any(m["id"] == model_registry_id for m in trained):
        raise HTTPException(status_code=404, detail=f"Model registry entry {model_registry_id} not found or not AVAILABLE")

    try:
        run_record = svc.create_pending_run(
            db=db,
            model_registry_id=model_registry_id,
            input_file_path=input_file_path,
            input_type=input_type,
            input_source=input_source,
        )
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc))
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))

    background_tasks.add_task(svc.run_inference_background, run_record.id)
    return JSONResponse(
        status_code=202,
        content={"run_id": run_record.id, "status": "PENDING", "message": "Inferencia iniciada en segundo plano."},
    )


@router.get("/inference-status/{run_id}")
def get_inference_status(run_id: int, db: Session = Depends(get_db)):
    """Poll the status of a supervised inference run."""
    try:
        return svc.get_run_status(db, run_id)
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc))
    except Exception as exc:
        raise _http_error(exc, 500)


# ── Read results ──────────────────────────────────────────────────────────────

@router.get("/prediction-runs")
def get_prediction_runs(db: Session = Depends(get_db)):
    """List all supervised inference runs."""
    try:
        return svc.list_prediction_runs(db)
    except Exception as exc:
        raise _http_error(exc, 500)


@router.get("/prediction-results")
def get_prediction_results(
    run_id: int = Query(...),
    page: int = Query(1, ge=1),
    page_size: int = Query(50, ge=1, le=500),
    priority_filter: str = Query("ALL"),
    sort_by: str = Query("prediction_probability"),
    db: Session = Depends(get_db),
):
    """Paginated results for a supervised inference run."""
    try:
        return svc.get_prediction_results(
            db,
            run_id=run_id,
            page=page,
            page_size=page_size,
            priority_filter=priority_filter,
            sort_by=sort_by,
        )
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc))
    except Exception as exc:
        raise _http_error(exc, 500)


@router.get("/prediction-report")
def get_prediction_report(
    run_id: int = Query(...),
    db: Session = Depends(get_db),
):
    """Summary report for a supervised inference run including priority distribution."""
    try:
        return svc.get_prediction_report(db, run_id=run_id)
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc))
    except Exception as exc:
        raise _http_error(exc, 500)


@router.get("/prediction-metadata")
def get_prediction_metadata(
    run_id: int = Query(...),
    db: Session = Depends(get_db),
):
    """Metadata for a supervised inference run and the model used."""
    try:
        return svc.get_prediction_metadata(db, run_id=run_id)
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc))
    except Exception as exc:
        raise _http_error(exc, 500)
