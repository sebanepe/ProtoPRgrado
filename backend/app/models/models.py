...
class Transaction(Base):
    ...
    merchant_hash = Column(String(255), nullable=True, index=True)
...
class RuleAlertReview(Base):
    """
    Tracks human reviews of rule-based alerts.
    Stores status changes for alerts generated by the rule engine.
    Non-destructive: original CSV files are never modified.
    """
    __tablename__ = "rule_alert_reviews"

    id = Column(Integer, primary_key=True, index=True)
    # Run identifier from CSV (e.g., "preprocessed_run_26")
    source_run = Column(String(255), nullable=False, index=True)
    # Alert identifiers (at least one must be present)
    alert_id = Column(String(255), nullable=True, index=True)
    summary_alert_id = Column(String(255), nullable=True, index=True)
    # Rule and transaction context
    rule_code = Column(String(255), nullable=False, index=True)
    transaction_id = Column(String(255), nullable=True)
    customer_hash = Column(String(255), nullable=True)
    # Status tracking
    previous_status = Column(String(50), nullable=True)
    new_status = Column(String(50), nullable=False, index=True)
    # Status values: NEW, IN_REVIEW, DISMISSED, FALSE_POSITIVE, CONFIRMED_FRAUD
    # Analyst information
    analyst_label = Column(String(50), nullable=True)  # e.g., "fraud", "false_positive", "dismissed"
    ...
    # Indexes for queries
    __table_args__ = (
        Index("ix_rule_alert_review_lookup", "source_run", "alert_id", "summary_alert_id"),
        Index("ix_rule_alert_review_status", "source_run", "new_status"),
        Index("ix_rule_alert_review_rule", "source_run", "rule_code"),
    )
